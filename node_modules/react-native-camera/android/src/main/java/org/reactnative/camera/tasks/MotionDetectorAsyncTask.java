...
private    WritableMap returnValue  = Arguments.createMap();
  ...
  private boolean isInInputArea(int x, int y){
    if (mMotionDetector.mArea == "") {
      ...
      if (areaValues[0] == "rectangle") {
        if( x < inputAreaLeft 
        ||  x > inputAreaWidth+mMotionDetector.mSampleSize
        ||  y < inputAreaTop
        ||  y > inputAreaHeight+mMotionDetector.mSampleSize
        ){
          return false;
        }
        else {
