  ...
  protected WritableMap doInBackground(Void... ignored) {
    ...
    try {
      ...
      ||  mMotionDetector.mPreviousSampledValues.length != bitmapWidth*bitmapHeight){
        ...
        // returnValue.putString("reset previous", bitmapWidth+"x"+bitmapHeight);
      }
      ...
      for ( int x = 0; x < bitmapWidth; x++) {
        for ( int y = 0; y < bitmapHeight; y++) {
          ...
          if (scoreRed>0 || scoreGreen>0 || scoreBlue>0){
            ...
            motionDetected = true;
            // We can stop here if denoising is not requested and if we are on run-mode.
          ...
          mMotionDetector.mPreviousSampledValues[i] = color;
          // returnValue.putString("Scan bitamp " + i, "x:"+x + " y:"+y);
          i++;
        } // for y
      } // for x
      bitmap = null;
      ...
      if (false && motionDetected && mMotionDetector.mMinimumPixels > 1){
        returnValue.putString("Denoising", "");
        motionDetected = false;
        breadcrumb = new boolean[bitmapWidth][bitmapHeight];
        returnValue.putString("Denoising", "breadcrumb set");
        ...
        for (i = 0; i < motionPixels.size(); i++) {
          ...
          if(!breadcrumb[x][y]) {
            ...
            if( -1== groupPixel(x, y, 0, bitmapWidth, bitmapHeight)){
               returnValue.putString("Error","group"); 
            }
