...
public class MotionDetectorAsyncTask extends android.os.AsyncTask<Void, Void, SparseArray<WritableMap>> {
  ...
  protected SparseArray<WritableMap> doInBackground(Void... ignored) {
    ...
    int i = 0;
pixel.putString("color","bitmap not ull");
int x=0;
int y=0;
int step = 3*5;
...
if (mMotionDetector.indexMotion>15) {
  ...
  @Override
  protected void onPostExecute(SparseArray<WritableMap> faces) {
    ...
    if (faces == null) {
      ...
      if (faces.size() > 0) {
        mDelegate.onMotionDetected(faces, mWidth, mHeight, mRotation);
      }
