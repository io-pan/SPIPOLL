...
import { distance, angle, center, centerContainerBased } from './utilities';
...
export class ViewEditor extends Component {
  ...
  _updatePanState(x = this.currentPanValue.x, y = this.currentPanValue.y) {
    ...
    console.log('pan _value', this.state.pan.x._value + ' '+ this.state.pan.y._value );
    console.log('pan _offset', this.state.pan.x._offset + ' '+ this.state.pan.y._offset );
  }
...
// console.log(e.nativeEvent.touches[0].locationX+ ' ' +e.nativeEvent.touches[0].locationY);
// console.log('pan.value', this.state.pan.x._value + ' '+ this.state.pan.y._value );
// console.log('pan.offset', this.state.pan.x._offset + ' '+ this.state.pan.y._offset );
// console.log('this._pan',this._pan);
// console.log(' ');

    const { imageContainerWidth, imageContainerHeight, imageWidth, imageHeight } = this.props;
    if (gestureState.numberActiveTouches === 1 && !this._multiTouch) {
      this.props.onChange(this.currentPanValue, this.currentScaleValue.value, this.currentAngleValue.value);
      ...
      this.state.angle.setValue( parseFloat(this._angle) + angleChange );
      ...
      // console.log('currentCenter', currentCenter.x+' - '+ currentCenter.y);
      ...
      const currentX = this._pan.x;
        // this._pan.x > 0 || newWidth < imageWidth ?
        //   0 : this._pan.x;
      const currentY = this._pan.y;
        // this._pan.y > 0 || newHeight < imageHeight ?
        //   0 : this._pan.y;

      // new image center
      let x = currentCenter.x - this._previousCenter.x + currentX;
      let y = currentCenter.y - this._previousCenter.y + currentY;
// console.log('touch center' ,  x +' - '+ y);


      // zoom to the center of container.

      if (this._previousScale === 0){
        this._previousScale = newScale;

      }
      else {
        x = this._pan.x + (this._pan.x / this._previousScale) * (newScale - this._previousScale);
        y = this._pan.y + (this._pan.y / this._previousScale) * (newScale - this._previousScale);
          ...
          if(angleChange){
            ...
            const tx =  Math.cos(rad) * dx - Math.sin(rad) * dy;
            const ty =  Math.sin(rad) * dx + Math.cos(rad) * dy;
            console.log('t ',tx+' '+ty);
            x += tx;
            y += ty;
          }
      ...
      if (!a){
         a=true;
          ...
          // const rad = (angleChange) * Math.PI / 180,
          // dx = Math.abs( imageContainerWidth/2 - x),
          // dy = Math.abs( imageContainerHeight/2 - y);
      ...
      // zoom to the center of touches.
   //    if (this._previousScale !== 0){

   //      // touch-center relative to container center.
   //      const currentCenterC =  centerContainerBased(e.nativeEvent.touches);
   //      let fx = currentCenterC.x 
   //              // + (this._pan.x>=0 ? 0 : this._pan.x)
   //              // + this._pan.x
   //              - imageContainerWidth/2
   //              ;
   //      let fy = currentCenterC.y 
   //              // + (this._pan.y>=0 ? 0 : this._pan.y)
   //              // + this._pan.y
   //              - imageContainerHeight/2
   //              ;

   // console.log('touch center' ,  fx +'   '+ fy);


   //      x +=  currentCenter.x - this._previousCenter.x + currentX
   //      y += currentCenter.y - this._previousCenter.y + currentY
   //    }
...
// Diff with center screen.


// fx=imageContainerWidth/2
// fy=imageContainerHeight/2

      // SET CENTER SCALE


      // // rotate to the center of the touches
      // if(angleChange){
...
// e.nativeEvent.locationY);
      ...
      // comme ça ça bouge pas et c'est mieux
      this.state.pan.setOffset({ x, y });
      this.state.pan.setValue({ x: 0, y: 0 });
  ...
  _handlePanResponderEnd(e) {
    const { imageWidth, imageHeight, isLandscape, isLong, isWide, maskWidth, maskHeight, maxZoomScale, checkAdjustment} = this.props;
    ...
    if (this._multiTouch) {
      ...
      this._previousCenter = 0;
      this._previousScale = 0;
      // Resize if don't fit minScale maxScale.
  ...
  render() {
    const { pan, scale, angle, render } = this.state;
    const {
      imageWidth,
      imageHeight,
      imageContainerWidth,
    ...
    const animatedStyle = {
      ...
      transform: [
        ...
        { scale },
        { rotate: !rotate ? 0 : angle.interpolate({
            inputRange: [0,360],
            outputRange: [initialRotate+'deg', (initialRotate+360)+'deg'],
          }) 
        },
      ]
    };
