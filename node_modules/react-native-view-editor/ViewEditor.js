...
export class ViewEditor extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      render: false,
        ...
        console.log('panLayout 0 ', layout);
    ...
    this._scale = 1,//containerDim / imageDim;
    // angle variables
  ...
  componentWillMount() {
    this._panResponder = PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (evt, gestureState) => true,
        ...
        const layout = this.state.pan.getLayout();
        console.log('panLayout 1 ', layout);
  }
  ...
  componentDidMount() {
    ...
    // this.state.pan.setOffset({ x: -60, y: 0 });
    // this._pan={x:-60, y:0}
    // this._updatePosition(-60,  0);
      this._checkAdjustment();
  }
  ...
  _handlePanResponderMove(e, gestureState) {
    ...
    if (gestureState.numberActiveTouches === 1 && !this._multiTouch) {
      ...
      // new image center  (to move image with 2 fingers)
      ...
      // rotate to center of container.
      if(angleChange ){
        const rad = (angleChange) * Math.PI / 180,
              tx =  Math.cos(rad) * x - Math.sin(rad) * y,
              ty =  Math.sin(rad) * x + Math.cos(rad) * y;
      ...
      // Trigger only on tuchEnds is enought
      // this.props.onChange(this.currentPanValue, this.currentScaleValue.value, this.currentAngleValue.value);
  ...
  _handlePanResponderEnd(e) {
    const { imageWidth, imageHeight, isLandscape, isLong, isWide, maskWidth, maskHeight, minZoomScale, maxZoomScale, checkAdjustment} = this.props;
    ...
    if (this._multiTouch) {
      ...
      if (this._scale < minZoomScale) {
        if (this.props.onZoomCallback) {
          this.props.onZoomCallback(false);
        }
        this._updateSize(minZoomScale);
...
//     console.log('panLayout', layout);
//     // console.log(' img', 'w:'+imageWidth + ' h:' +imageHeight );

// console.log(' _pan:',this._pan);
// console.log(' state pan', this.state.pan);
